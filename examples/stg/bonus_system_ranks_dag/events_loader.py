...
class EventLoader:
    ...
    LAST_LOADED_ID_KEY_EVENT = "last_loaded_id"
    ...
    def load_events(self):
        ...
        with self.pg_dest.connection() as conn:
            ...
            if not wf_setting:
                wf_setting = EtlSetting(id=0, workflow_key=self.WF_KEY, workflow_settings={self.LAST_LOADED_ID_KEY_EVENT: -1})
            last_loaded = wf_setting.workflow_settings[self.LAST_LOADED_ID_KEY_EVENT]
            ...
            wf_setting.workflow_settings[self.LAST_LOADED_ID_KEY_EVENT] = max([t.id for t in load_queue])
            wf_setting_json = json2str(wf_setting.workflow_settings)  # Преобразуем к строке, чтобы положить в БД.
            self.settings_repository.save_setting(conn, wf_setting.workflow_key, wf_setting_json)

            self.log.info(f"Load finished on {wf_setting.workflow_settings[self.LAST_LOADED_ID_KEY_EVENT]}")
